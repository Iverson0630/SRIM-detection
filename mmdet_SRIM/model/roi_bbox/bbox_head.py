...
class BBoxHead(BaseModule):
    ...
    def __init__(self,
        ...
        self.loss_class_cls = build_loss(loss_class_cls)
        ...
        if self.with_cls:
            ...
            if self.custom_cls_channels:
                cls_channels = self.loss_class_cls.get_cls_channels(self.num_classes)
            else:
    ...
    def custom_cls_channels(self):
        return getattr(self.loss_class_cls, 'custom_cls_channels', False)

    @property
    def custom_activation(self):
        return getattr(self.loss_class_cls, 'custom_activation', False)

    @property
    def custom_accuracy(self):
        return getattr(self.loss_class_cls, 'custom_accuracy', False)
    ...
    def loss(self,
        ...
        if cls_score is not None:
            ...
            if cls_score.numel() > 0:
                ...
                loss_domain_cls_ = self.loss_domain_cls(
                    cls_score,
                    labels,
                    label_weights,
                    avg_factor=avg_factor,
                ...
                loss_class_cls_ = self.loss_class_cls(
                    cls_score,
                    labels,
                    label_weights,
                    avg_factor=avg_factor,
                ...
                if isinstance(loss_class_cls_, dict):
                    losses.update(loss_class_cls_)
                    losses.update(loss_domain_cls_)
                else:
                    losses['loss_class_cls'] = loss_class_cls_
                    losses['loss_domain_cls'] = loss_domain_cls_
                if self.custom_activation:
                    acc_ = self.loss_class_cls.get_accuracy(cls_score, labels)
                    losses.update(acc_)
                else:
                    losses['acc_domain'] = accuracy(cls_score, labels, True)
                    losses['acc_class'] = accuracy(cls_score, labels, False)
    ...
    def get_bboxes(self,
        ...
        if self.custom_cls_channels:
            scores = self.loss_class_cls.get_activation(cls_score)
        else:
            scores = F.softmax(
