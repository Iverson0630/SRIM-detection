...
yolo_format_classes_path='yolo/labels/classes.txt'     #类别文件，一行一个类
yolo_format_annotation_path='yolo/labels/val'        #yolo格式标签所在文件夹
img_pathDir='leftImg8bit/val'                        #图片所在文件夹
...
box_id = 0
for imageFiles in imageFileList:
    imageFiles_ =  os.listdir(os.path.join(img_pathDir,imageFiles))
    for i,imageFile in enumerate(imageFiles_):
        if imageFile[0]=='.':
            break
        imagePath = os.path.join(img_pathDir,imageFiles,imageFile)                             #获取图片的绝对路径
        ...
        img_context['file_name']=os.path.join(imageFiles,imageFile)
        ...
        with open(os.path.join(yolo_format_annotation_path,imageFiles,txtFile),'r') as fr:
